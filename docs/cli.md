...
Matches identify only the public file and category. The private pattern is never repeated. The validator also checks complete Repository Contracts, required hardware evidence, real ISO evidence dates for schedulable nodes and Roles, repository-native agent contracts, local links, actual rendering with the pinned Mermaid CLI, source/SVG/PNG manifest hashes, rendered dimensions, and generated text artifacts. Mermaid source hashes use canonical LF line endings so the same commit validates from LF and CRLF Git checkouts; committed SVG and PNG hashes remain exact-byte checks.
...
The private probe configuration supplies a peer target, SSH destination, expected root-disk encryption policy, and minimum free-space budget from the Private Operations Overlay. The network gate requires a peer ping, a successful key-only SSH login, and key-only daemon policy. Use `--role-profile workstation` for DEV. The collector does not install packages, pull images, or change configuration. It inspects SMART health, encryption/headroom, time synchronization, pending upgrades, firewall state, an issue-owned linked worktree, and a preloaded `busybox:1.36` container before applying Role-specific checks. It verifies the source issue is open, has exactly one assignee, has no open native blockers, and binds the exact worktree branch with a complete valid Worktree binding. It also performs bounded CPU, memory, host-GPU, and `--pull=never --rm` GPU-container execution where those gates apply. Browser acceleration is checked through Chromium's GPU diagnostic page, separately from the OpenGL graphics smoke test. Command output is written as `private_evidence`; stdout reports counts only. Zero exit status alone is insufficient: each probe output must satisfy its named semantic contract. Unsupported, unavailable, timed-out, or semantically incomplete probes remain gated. The `fixture` adapter requires both `--probe-results` and matching `--issue-evidence` for deterministic tests and does not contact a machine.
...
Node Slot, reusable Role profile, and OS Profile are separate inputs. A future `compute-02` can reuse `--role-profile compute`; a workstation uses `--role-profile workstation`. Run the command from the issue worktree and place `--replay-ledger` outside it. The ledger is persistent private control state: the evaluator atomically claims the collector-generated observation UUID, and a second evaluation of that UUID fails closed. The default `public` view emits only controlled check/status summaries, never caller-authored evidence or `private_evidence`; obvious private identity in a public-evidence input is also rejected. `--view private` is an explicit operator action and its output must stay in the Private Operations Overlay workflow.
...
The live verifier confirms that the issue number embedded in the branch is open, assigned, free of open native blockers, and bound to that exact branch under `### Worktree binding`. The selected worktree's normalized `origin` must resolve to that same repository. Deterministic tests may use `--issue-evidence` with the same authority fields instead. The selected path must be a clean, registered linked worktree whose current issue branch and `HEAD` exactly match the request. Review, test, health, and rollback JSON each require `status: pass` and the same source commit. Without `--deployment-authorized`, the command fails before writing a receipt. The receipt contains hashes and verification results, not local paths or evidence-file contents. Real-machine pilot acceptance remains blocked until the live admission issues pass.
